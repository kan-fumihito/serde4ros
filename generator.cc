...
void generate_header(const ROSMsgType &msg) {
    FILE *fp;
      ...
      fp = fopen(std::string("include/std_msgs/" + type + ".h").c_str(), "w");
    ...
    public_rows.emplace_back("  "+type+"(const "+type+"&) = delete;\n");
    public_rows.emplace_back("  "+type+"& operator=(const "+type+"&) = delete;\n");
    public_rows.emplace_back("  "+type+"& operator=("+type+"&&) = delete;\n\n");
    public_rows.emplace_back("  "+type+"() = default;\n");
    public_rows.emplace_back("  "+type+"("+type+"&&) = default;\n\n");
    public_rows.emplace_back("  void serialize(std::string &fname);\n");
    public_rows.emplace_back("  void serialize_" + type +
                             "(void);\n");
    public_rows.emplace_back("  void deserialize(std::string &fname);\n");
    public_rows.emplace_back("  void deserialize_" + type +
                             "(void);\n");
...
void generate_serializer(const ROSMsgType &msg) {
  FILE *fp;
    ...
    auto type = msg.getTypeName();
    fp = fopen(std::string("src/std_msgs/" + type + "Serialize.cc").c_str(), "w");
    fprintf(fp, "#include\"std_msgs/%s.h\"\n\n", type.c_str());
